...
gulp.task('playground', (cb) => {
  ...
  let content = fs.readFileSync(path.resolve(__dirname, '../playground/index.php'), 'utf8');
  if (env === 'development') {
    content = content
  ...
  fs.writeFileSync(path.resolve(__dirname, '../playground/index.php'), content);
  if (cb) cb();
});
gulp.task('js', (cb) => {
  buildJs(cb);
...
gulp.task('open', () => {
  gulp.src('./playground/index.php').pipe(gopen({ uri: 'http://localhost:3000/playground/' }));
});
